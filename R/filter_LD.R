...
#'
#' Filter LD
#' @param LD_list List containing SNP-level data (\code{query_dat}),
...
#' will be removed from both \code{query_dat} and \code{LD} list items..
...
#' LD_list2 <- echoLD::filter_LD(LD_lis = LD_listt, min_r2 = .2)
filter_LD <- function(LD_list,
   ...
   query_dat<- LD_list$query_dat
    ...
    if (all((min_r2 != 0) & (min_r2 != FALSE))) {
       ...
       query_dat<- LD_list$query_dat
        LD_matrix <- LD_list$LD
        lead.snp <- subset(query_dat, leadSNP)$SNP[1]
